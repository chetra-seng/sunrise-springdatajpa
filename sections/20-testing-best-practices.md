...
class TaskRepositoryTest {
    ...
    void findByCompleted_returnsOnlyMatchingTasks() {
        taskRepository.save(TaskModel.builder().title("Done task").completed(true).build());
        taskRepository.save(TaskModel.builder().title("Open task").completed(false).build());

        List<TaskModel> completed = taskRepository.findByCompleted(true);
...
void findById_returnsTask_whenExists() {
    TaskModel saved = taskRepository.save(
        TaskModel.builder().title("Test task").completed(false).build()
    );

    Optional<TaskModel> found = taskRepository.findById(saved.getId());
...
void findById_returnsEmpty_whenNotFound() {
    Optional<TaskModel> found = taskRepository.findById(999L);
...
void deleteById_removesTask() {
    TaskModel saved = taskRepository.save(
        TaskModel.builder().title("To delete").completed(false).build()
    );
...
void findByCompleted_returnsBothGroups() {
    taskRepository.save(TaskModel.builder().title("A").completed(true).build());
    taskRepository.save(TaskModel.builder().title("B").completed(true).build());
    taskRepository.save(TaskModel.builder().title("C").completed(false).build());

    List<TaskModel> completedTasks = taskRepository.findByCompleted(true);
    List<TaskModel> openTasks     = taskRepository.findByCompleted(false);
...
void findByCompleted_withEntityManager() {
    entityManager.persist(TaskModel.builder().title("Done").completed(true).build());
    entityManager.flush();   // forces INSERT immediately

    List<TaskModel> results = taskRepository.findByCompleted(true);
